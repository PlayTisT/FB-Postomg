...
test('step 1 Login-Facebook', async ({ page }) => {
 // step 1 go to FB page and login.
  ...
  await test.step('Step 2 FB Post ', async () => {
    //await page.getByRole('link', { name:'Orkan Porabear' }).click(); // กลับไปหน้าแรก Profile FB
    //await page.goto('https://www.facebook.com/profile.php?');
    //await page.getByRole('button', { name: 'What\'s on your mind?' }).waitFor();
    ...
    //await page.getByLabel('What\'s on your mind?').fill('สวัสดีจ้า Playwright 55');
